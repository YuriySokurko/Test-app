...
-(void)drawRect:(CGRect)rect{
    ...
    UIFont *font=[UIFont fontWithName: FontName size: [self scaleAccordingToDeviceSize: FontSize]];
...
- (CGFloat)scaleAccordingToDeviceSize: (CGFloat)maximumSize {
    CGFloat result = maximumSize;
    
    if ([UIScreen mainScreen].bounds.size.height == 480) {
        // iPhone 4
        result = maximumSize * 0.6;
    } else if ([UIScreen mainScreen].bounds.size.height == 568) {
        // IPhone 5
        result = maximumSize * 0.83;
    } else if ([UIScreen mainScreen].bounds.size.height == 812) {
        // IPhone X
        result = maximumSize * 0.95;
    } else if ([UIScreen mainScreen].bounds.size.width == 375) {
        // iPhone 6
        result = maximumSize * 0.90;
    } else if ([UIScreen mainScreen].bounds.size.width == 414) {
        // iPhone 6+
        result = maximumSize * 1;
    } else if ([UIScreen mainScreen].bounds.size.width == 768) {
        // iPad
        result = maximumSize * 1.2;
    }
...
-(BOOL)validate {
    ...
    self.backgroundView.backgroundColor = ColorBackground;
    self.backgroundView.layer.shadowColor = (__bridge CGColorRef _Nullable)(ColorBackground);
    self.tintColor = ColorBackground;
...
- (void)showErrorIconForMsg:(NSString *)msg {
    UIButton *btnError=[[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width - self.frame.size.height/2, self.center.y - self.frame.size.height/4, self.frame.size.height/2, self.frame.size.height/2)];
    ...
    self.backgroundView.backgroundColor = ColorPopUpBg;
    self.backgroundView.layer.shadowColor = (__bridge CGColorRef _Nullable)(ColorPopUpBg);
    self.tintColor = ColorPopUpBg;
    self.rightViewMode = UITextFieldViewModeAlways;
    self.textColor = ColorPopUpBg;
